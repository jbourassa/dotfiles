...
require('lazy').setup({
  ...
  -- ^ or v
  -- 'vim-ruby/vim-ruby',
  'tpope/vim-sleuth', -- Detect tabstop and shiftwidth automatically – already in polyglot
  'jlcrochet/vim-ruby',

  'vim-scripts/bufkill.vim', -- :BD & friends
  'AndrewRadev/splitjoin.vim', -- gS and gJ

  'tpope/vim-endwise', -- automatically add ruby "end", but.. doesn't work?
...
-- No annoyance when multiple files are opened
vim.o.backup = false
...
-- Always have 5 lines at the top/bottom
vim.o.scrolloff = 5
...
vim.keymap.set('v', '<leader>y', '"+y', { desc = "[Y]ank to system clipboard", silent = true })

vim.keymap.set('n', '<Leader>cp', ':let @*=expand("%")<CR>',   { desc = "[C]opy relative [p]ath" })
...
vim.keymap.set('x', '.', ':norm.<CR>', { desc = "Repeat last operation to selected lines" })
...
require('telescope').setup {
  defaults = {
    layout_strategy = 'vertical', -- deal with long file names
    mappings = {
      i = {
        ['<C-u>'] = false,
        ['<C-d>'] = false,
...
local on_attach = function(_, bufnr)
  ...
  -- @TODO I probably want those, but not under <leader>w
